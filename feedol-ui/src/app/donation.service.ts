...
export class DonationService {
 ...
 saveRestaurant(restaurant){
   var saveRestaurantUrl= "http://13.234.235.193:80/registration-service/api/v1/restaurant-profile/";
   console.log("Saving Restaurant", restaurant);
   return this.http.post<any>(saveRestaurantUrl,restaurant);
 }

 saveCharity(charity){
  var saveCharityUrl= "http://13.234.235.193:80/registration-service/api/v1/charity-profile/";
  console.log("Saving Charity", charity);
  return this.http.post<any>(saveCharityUrl,charity);
}

saveDeliveryBoy(deliveryBoy){
  var saveDeliveryBoyUrl= "http://13.234.235.193:80/registration-service/api/v1/deliveryBoy-profile/";
 ...
 updateRestaurantActivity(details){
  var url= "http://13.234.235.193:80/routing-service/updateRestaurantDetails";
 ...
 updateDeliveryBoyActivity(details){
  var url= "http://13.234.235.193:80/routing-service/updateDeliveryBoyDetails";
 ...
 fetchRestaurantLogs(username){
   var url= `http://13.234.235.193:80/restaurant-service/api/v1/restaurant-logs/?username=${username}`;
   console.log("Fetching Restaurant's Logs With Username : ", username);
   return this.http.get<any>(url);
  }
 
  fetchRestaurantFoodStatus(username){
   var url= `http://13.234.235.193:80/restaurant-service/api/v1/restaurant-status/?username=${username}`;
   console.log("Fetching Restaurant's Food Status With Username : ", username);
   return this.http.get<any>(url);
  }
 
 fetchCharityLogs(username){
   var url= `http://13.234.235.193:80/charity-service/api/v1/charity-logs/?username=${username}`;
   console.log("Fetching Charity's Logs With Username : ", username);
   return this.http.get<any>(url);
  }
 
 fetchCharityFoodStatus(username){
  var url= `http://13.234.235.193:80/charity-service/api/v1/charity-status/?username=${username}`;
  console.log("Fetching Charity's Food Status With Username : ", username);
  return this.http.get<any>(url);
 }
 
 fetchDeliveryBoyLogs(username){
   var url= `http://13.234.235.193:80/deliveryboy-service/api/v1/deliveryBoy-logs/?username=${username}`;
   console.log("Fetching Delivery Boy's Route Status With Username : ", username);
   return this.http.get<any>(url);
  }
 
 fetchDeliveryBoyRoute(username){
   var url= `http://13.234.235.193:80/deliveryboy-service/api/v1/deliveryBoy-status/?username=${username}`;
   console.log("Fetching Delivery Boy's Route Status With Username : ", username);
   return this.http.get<any>(url);
  }

  saveRestaurantLogOnPicked(username):Observable<any>{
    var url= `http://13.234.235.193:80/restaurant-service/api/v1/restaurant-logs`;
  ...
  saveCharityLogOnDelivered(username){
    var url= `http://13.234.235.193:80/charity-service/api/v1/charity-logs/`;
    console.log("Saving Charity Log With Username : ", username);
    return this.http.post<any>(url,username);
   }
  
  onRate(rating : Rating){
    var url= `http://13.234.235.193:80/charity-service/api/v1/charity-rating/`;
